...
def validate_input_data():
    ...
    with fs.open('gs://gcs-mlops/advertising_roi/campaign_spend.csv') as f:
        df1 = pd.read_csv(f)

    with fs.open('gs://gcs-mlops/advertising_roi/monthly_revenue.csv') as f:
        df2 = pd.read_csv(f)
...
def read_last_training_metrics():
    ...
    table_id = "dataanalytics-347914.ml_ops.advertising_roi_model_metrics"
    query = f"""
        SELECT *
        FROM `{table_id}`
...
def continuous_training():
    ...
    campaign_file_path = "gs://gcs-mlops/advertising_roi/campaign_spend.csv"
    df_spend = read_campaign_data(campaign_file_path)

    revenue_file_path = "gs://gcs-mlops/advertising_roi/monthly_revenue.csv"
